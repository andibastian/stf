...
function getDevices() {
  return db.run(r.table("devices")).then(function (cursor) {
...
dbapi.updateBootStrap = function (rootGroup, env) {
  ...
  function createNewAdminUser() {
    ...
    return dbapi
      .createUser(env.STF_ADMIN_EMAIL, env.STF_ADMIN_NAME, "127.0.0.1")
      .then(function (stats) {
        if (!stats.inserted) {
        ...
        log.info(
          "Created (user name:%s email:%s)",
          env.STF_ADMIN_NAME,
          env.STF_ADMIN_EMAIL
        );

        return dbapi
          .loadUser(rootGroup.owner.email)
          .then(function (oldAdminUser) {
            return db.run(
              r.table("users").get(env.STF_ADMIN_EMAIL).update({
          ...
          r
            ...
            .update({
              group: {
                name: r.branch(
                  r
                    .expr(rootGroupNameHasChanged)
                    .eq(true)
                    .and(r.row("group")("id").eq(rootGroup.id)),
                ...
                originName: r.branch(
                  r
                    .expr(rootGroupNameHasChanged)
                    .eq(true)
                    .and(r.row("group")("origin").eq(rootGroup.id)),
                  env.STF_ROOT_GROUP_NAME,
                  r.row("group")("originName")
                ),
                owner: r.branch(
                  r
                    .expr(adminEmailHasChanged)
                    .eq(true)
                    .and(r.row("group")("id").eq(rootGroup.id)),
                  {
  ...
  function updateGroupsForMigration() {
    if (rootGroupNameHasChanged && !adminEmailHasChanged) {
      return db.run(
        r.table("groups").get(rootGroup.id).update({
            ...
            .update({
              ...
              owner: r.branch(
                r
                  .expr(adminEmailHasChanged)
                  .eq(true)
                  .and(r.row("owner")("email").eq(rootGroup.owner.email)),
                {
                  name: env.STF_ADMIN_NAME,
                  email: env.STF_ADMIN_EMAIL,
                },
              ...
              users: r.branch(
                r.expr(adminEmailHasChanged).eq(true),
                _.union(
                  [env.STF_ADMIN_EMAIL],
                  _.difference(group.users, [rootGroup.owner.email])
                ),
                r.row("users")
              ),
            })
  ...
  return createNewAdminUser()
    .then(function (success) {
      ...
      return updateGroupsForMigration()
        .then(function () {
    ...
    .catch(function (err) {
      log.error(
        "Failed to update built-in objects, potential data consistency issue"
      );
      return Promise.reject(err);
    });
};
...
dbapi.getGroupsByIndex = function (value, index) {
  return db
    .run(r.table("groups").getAll(value, { index: index }))
    .then(function (cursor) {
      return cursor.toArray();
    });
};
...
dbapi.getGroupByIndex = function (value, index) {
  return dbapi.getGroupsByIndex(value, index).then(function (array) {
    return array[0];
  });
};
...
dbapi.getGroupsByUser = function (email) {
  return db
    .run(
      r.table("groups").filter(function (group) {
...
dbapi.getGroups = function () {
  return db.run(r.table("groups")).then(function (cursor) {
    return cursor.toArray();
  });
};

dbapi.getUsers = function () {
  return db.run(r.table("users")).then(function (cursor) {
    return cursor.toArray();
  });
};
  ...
  return Promise.all([
    ...
    db.run(
      r
        ...
        .get(email)
        .update({
          groups: { subscribed: r.row("groups")("subscribed").setInsert(id) },
        })
    ),
  ...
  return Promise.all([
    ...
    db.run(
      r
        ...
        .update({
          groups: {
            subscribed: r.row("groups")("subscribed").setDifference([id]),
          },
        })
    ),
  ]).then(function () {
...
dbapi.lockBookableDevice = function (groups, serial) {
  function wrappedlockBookableDevice() {
    return db
              ...
              group: {
                lock: r.branch(
                  r
                    .row("group")("lock")
                    .eq(false)
                    .and(r.row("group")("class").ne(apiutil.STANDARD))
                    .and(
                      r
                        .expr(groups)
      ...
      .then(function (stats) {
        return apiutil.lockDeviceResult(
          stats,
          dbapi.loadBookableDevice,
          groups,
          serial
        );
      });
...
dbapi.addGroupDevices = function (group, serials) {
  ...
  return dbapi
    .updateUserGroupDuration(group.owner.email, group.duration, duration)
    .then(function (stats) {
      if (stats.replaced) {
...
dbapi.lockUser = function (email) {
  function wrappedlockUser() {
    return setLockOnUser(email, true).then(function (stats) {
                    ...
                    r
                      ...
                      .and(
                        r
                          .row("owner")("email")
                          .eq(email)
                          .or(r.expr(email).eq(group.owner.email))
                      ),
                    true,
                    r.row("lock")("user")
                  ),
  ...
  function wrappedAdminLockGroup() {
    return db
      ...
      .then(function (stats) {
        ...
        if (stats.replaced) {
          result.status =
            stats.changes[0].new_val.lock.admin &&
            !stats.changes[0].old_val.lock.user;
          if (result.status) {
            result.data = true;
...
dbapi.getRootGroup = function () {
  return dbapi
    .getGroupByIndex(apiutil.ROOT, "privilege")
    .then(function (group) {
      if (!group) {
...
dbapi.createUserGroup = function (data) {
  return dbapi
    .reserveUserGroupInstance(data.owner.email)
    .then(function (stats) {
      if (stats.replaced) {
...
dbapi.updateGroup = function (id, data) {
  return db.run(r.table("groups").get(id).update(data)).then(function () {
    return dbapi.getGroup(id);
  });
};
        ...
        groups: {
          quotas: {
            consumed: {
              number: r.branch(
                r
                  .row("groups")("quotas")("consumed")("number")
                  .add(1)
                  .le(r.row("groups")("quotas")("allocated")("number")),
                r.row("groups")("quotas")("consumed")("number").add(1),
        ...
        groups: {
          quotas: {
            consumed: {
              duration: r.branch(
                r
                  .row("groups")("quotas")("consumed")("duration")
                  .sub(oldDuration)
                  .add(newDuration)
              ...
              allocated: {
                ...
                number: r.branch(
                  r
                    ...
                    .ne(null)
                    .and(
                      r.row("groups")("quotas")("consumed")("number").le(number)
                    )
                    .and(
                      r
                        .expr(duration)
...
dbapi.updateDefaultUserGroupsQuotas = function (
  email,
  duration,
  number,
  repetitions
...
dbapi.updateDeviceCurrentGroupFromOrigin = function (serial) {
  return db.run(r.table("devices").get(serial)).then(function (device) {
    return db
      .run(r.table("groups").get(device.group.origin))
      .then(function (group) {
        return db.run(
          r
            .table("devices")
...
dbapi.updateUserGroup = function (group, data) {
  return dbapi
    .updateUserGroupDuration(group.owner.email, group.duration, data.duration)
    .then(function (stats) {
      if (
        stats.replaced ||
        (stats.unchanged && group.duration === data.duration)
      ) {
...
dbapi.deleteUserGroup = function (id) {
  function deleteUserGroup(group) {
    return dbapi
      ...
      .then(function () {
        return dbapi.updateUserGroupDuration(
          group.owner.email,
          group.duration,
          0
        );
          ...
          r.table("users").insert(
            {
              ...
              groups: {
                ...
                quotas: {
                  ...
                  defaultGroupsNumber: adminUser
                    ? 0
                    : group.envUserGroupsNumber,
                  defaultGroupsDuration: adminUser
                    ? 0
                    : group.envUserGroupsDuration,
                  defaultGroupsRepetitions: adminUser
                    ? 0
                    : group.envUserGroupsRepetitions,
...
dbapi.saveUserAfterLogin = function (user) {
  return db.run(r.table("users").get(user.email)).then(function (oldUser) {
    ...
    return db.run(
      r.table("users").get(user.email).update(
        {
          ip: user.ip,
          lastLoggedInAt: r.now(),
        },
...
dbapi.lookupUsersByAdbKey = function (fingerprint) {
  return db.run(
    r.table("users").getAll(fingerprint, {
      index: "adbKeys",
    })
  );
};
...
dbapi.loadUserDevices = function (email) {
  return db
    .run(r.table("users").get(email).getField("groups"))
    .then(function (groups) {
      return db.run(
        r.table("devices").filter(function (device) {
          return r
            .expr(groups.subscribed)
...
dbapi.saveDeviceInitialState = function (serial, device) {
  ...
  return db
    .run(r.table("devices").get(serial).update(data))
    .then(function (stats) {
      if (stats.skipped) {
        return dbapi.getRootGroup().then(function (group) {
          data.serial = serial;
  ...
  return db.run(
    r
      ...
      .update({
        ...
        statusChangedAt: r.branch(
          r.expr(statusTimeStamp).gt(r.row("statusTimeStamp")),
          r.now(),
          r.row("statusChangedAt")
        ),
        statusTimeStamp: r.branch(
          r.expr(statusTimeStamp).gt(r.row("statusTimeStamp")),
          statusTimeStamp,
...
dbapi.loadAccessTokens = function (email) {
  return db.run(
    r.table("accessTokens").getAll(email, {
      index: "email",
    })
  );
};
...
dbapi.loadAccessToken = function (id) {
  ...
  const result = db.run(r.table("accessTokens").get(id));
  return result;
};
dbapi.loadUserByToken = function (tokenId) {
  return dbapi.loadAccessToken(tokenId).then(function (token) {
    if (token && token.user && token.user.email) {
      return dbapi.loadUser(token.user.email);
    }
    return null;
  });
};

dbapi.loadTokenLists = function () {
  return db.run(r.table("accessTokens").orderBy(r.desc("createdAt")));
};
