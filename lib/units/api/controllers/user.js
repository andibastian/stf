...
function getUserDevices(req, res) {
  ...
  dbapi
    .loadUserDevices(req.user.email)
...
function getUserDeviceBySerial(req, res) {
  ...
  dbapi
    .loadDevice(req.user.groups.subscribed, serial)
    .then(function (cursor) {
...
function addUserDevice(req, res) {
  var serial = req.hasOwnProperty("body")
    ? req.body.serial
    : req.swagger.params.serial.value;
  var timeout = req.hasOwnProperty("body")
    ? req.body.timeout || null
    : req.swagger.params.timeout.value || null;
  const lock = {};

  lockutil
    .lockGenericDevice(req, res, lock, dbapi.lockDeviceByCurrent)
    .then(function (lockingSuccessed) {
      if (lockingSuccessed) {
        ...
        var responseTimer = setTimeout(function () {
          req.options.channelRouter.removeListener(
            wireutil.global,
            messageListener
          );
          lockutil.unlockDevice(lock);
    ...
    .catch(function (err) {
      lockutil.unlockDevice(lock);
      apiutil.internalError(
        res,
        `Failed to take control of ${serial} device: `,
        err.stack
      );
    });
}
...
function getUserAccessTokens(req, res) {
  dbapi
    .loadAccessTokens(req.user.email)
    .then(function (cursor) {
      return Promise.promisify(cursor.toArray, cursor)().then(function (list) {
        var titles = [];
  ...
  adb.util
    ...
    .then(function (data) {
      if (data.users.length) {
        ...
        return dbapi
          .insertUserAdbKey(req.user.email, data.key)
          .then(function () {
            return res.json({
              success: true,
            });
...
function getAccessToken(req, res) {
  ...
  dbapi
    ...
    .catch(function (err) {
      apiutil.internalError(
        res,
        'Failed to delete access token "%s": ',
        id,
        err.stack
      );
    });
  ...
  dbapi
    ...
    .then(function (cursor) {
      Promise.promisify(cursor.toArray, cursor)().then(function (tokens) {
        ...
        apiutil.respond(res, 200, "Access Tokens Information", {
          tokens: tokenList,
        });
      });
    })
...
function createAccessToken(req, res) {
  ...
  dbapi
    .saveUserAccessToken(req.user.email, {
      title: title,
      id: id,
      jwt: jwt,
    })
    ...
    .catch(function (err) {
      apiutil.internalError(
        res,
        'Failed to create access token "%s": ',
        title,
        err.stack
      );
    });
...
function deleteAccessTokens(req, res) {
  dbapi
    .removeUserAccessTokens(req.user.email)
    .then(function (stats) {
      if (!stats.deleted) {
