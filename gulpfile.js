...
gulp.task("jsonlint", function () {
  return gulp
    .src([".bowerrc", ".yo-rc.json", "*.json"], { allowEmpty: true })
    .pipe(jsonlint())
...
gulp.task("eslint", function () {
  return (
    gulp
      .src(["lib/**/*.js", "res/**/*.js", "!res/bower_components/**", "*.js"])
...
gulp.task(
  "protractor",
  gulp.series("webdriver-update", function (callback) {
    gulp
      .src(["./res/test/e2e/**/*.js"])
      .pipe(
...
gulp.task("webpack:build", function (callback) {
  ...
  webpack(myConfig, function (err, stats) {
    ...
    fromString("stats.json", JSON.stringify(stats.toJson())).pipe(
      gulp.dest("./tmp/")
    );
...
gulp.task("pug", function () {
  return gulp
    .src(["./res/**/*.pug", "!./res/bower_components/**"])
    .pipe(
      pug({
        locals: {
...
gulp.task("clean", function (cb) {
  return del(["./tmp", "./res/build", ".eslintcache"], cb);
});
...
gulp.task("watch", function () {
  gulp.watch("res/**/*.pug", gulp.series("pug"));
  gulp.watch("res/**/*.styl", gulp.series("webpack:build")); // or stylus compiler if split
  gulp.watch("res/**/*.js", gulp.series("webpack:build"));
});
